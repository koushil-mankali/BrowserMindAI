...
const SummarizerComponent = () => {
  ...
  useEffect(() => {
    try {
      (async () => {
        ...
        //@ts-expect-error Summarizer is a global object provided by the chrome browser
  ...
  const summarizeHandler = async () => {
    ...
    try {
      ...
      const summarizedAIOutput = await Summarizer.create({
        sharedContext:
      ...
      const summarizedOutput = await summarizedAIOutput.summarize(inputText);

      setOutputText(summarizedOutput);
      summarizedAIOutput?.destroy();
    } catch (err: any) {
      setStatus("❌");
      setErrorMessage(err?.message);
    }
  ...
  return (
    ...
    >
      ...
      >
        <p>Summarizer AI Status: {status}</p>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
        ...
        >
          {loader ? "Summarizing..." : "Summarize"}
        </button>
      </div>
      <div style={{ color: "green" }}>
        <p style={{ marginBottom: "3px" }}>AI Summary Output:</p>{" "}
